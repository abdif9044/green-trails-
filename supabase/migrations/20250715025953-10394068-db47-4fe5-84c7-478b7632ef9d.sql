-- Fix the handle_new_user trigger to work without user_stats table
...
BEGIN
  ...
  -- Create user_stats record if the table exists, otherwise skip
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user_stats') THEN
    INSERT INTO public.user_stats (user_id, total_distance, total_elevation, total_trails, current_streak)
