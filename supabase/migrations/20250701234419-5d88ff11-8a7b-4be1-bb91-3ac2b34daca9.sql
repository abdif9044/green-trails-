...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.check_constraints WHERE constraint_name = 'trails_rating_check') THEN
    ALTER TABLE public.trails ADD CONSTRAINT trails_rating_check CHECK (rating >= 0 AND rating <= 5);
  END IF;
END $$;
