...
serve(async (req) => {
  ...
  try {
    ...
    const openAIApiKey = 'c10ac85b-aaf8-428b-b7cd-ffe342769805'; // onX key for OpenAI
    const weatherApiKey = '2f6fe1dd36e9425a3a51a182d9d9b3ca'; // OpenWeather key
    
    console.log('🌤️ Weather Prophet - Production keys configured');
    ...
    // Fetch extended weather forecast with production key
    ...
    // Call OpenAI for analysis with production key
    ...
    return new Response(
      JSON.stringify({ 
        ...
        success: false,
        fallback_message: 'Weather analysis temporarily unavailable. Please check back soon.'
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }, 
        status: 500 
