...
-- Create comprehensive profile policies with new names
-- Users can view basic public profile info of others
...
CREATE POLICY "Allow admins to manage trails" ON public.trails
FOR ALL
TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS "Admins can delete reviews" ON public.trail_reviews;
CREATE POLICY "Allow admins to delete reviews" ON public.trail_reviews
FOR DELETE
TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS "Admins can manage trail waypoints" ON public.trail_waypoints;
CREATE POLICY "Allow admins to manage trail waypoints" ON public.trail_waypoints
FOR ALL
TO authenticated
USING (public.is_admin());

DROP POLICY IF EXISTS "Admins can manage trail 3d models" ON public.trail_3d_models;
CREATE POLICY "Allow admins to manage trail 3d models" ON public.trail_3d_models
FOR ALL
TO authenticated
USING (public.is_admin());
